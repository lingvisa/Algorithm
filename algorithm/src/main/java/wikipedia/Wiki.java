...
public class Wiki implements Serializable
{
    ...
    public String getNSentInPage(String title, int sentCount) throws IOException
    {
        // pitfall check
        if (namespace(title) < 0)
            throw new UnsupportedOperationException("Cannot retrieve Special: or Media: pages!");
        ...
        // go for it
        // &prop=extracts&exsentences=1&titles=
        String url = apiUrl + "action=query&prop=extracts&explaintext=true&exsentences=" +sentCount + "&titles=" + URLEncoder.encode(normalize(title), "UTF-8");
        String temp = fetch(url, "getNSentInPage");
        //log(Level.INFO, "getNSentInPageText", "Successfully retrieved text of " + title);
        return temp;
    }
